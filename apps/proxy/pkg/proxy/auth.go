...
func (p *Proxy) Authenticate(ctx *gin.Context, boxIdOrSignedToken string, port float32) (boxId string, didRedirect bool, err error) {
	...
	cookieBoxId, err := ctx.Cookie(SANDBOX_AUTH_COOKIE_NAME + boxIdOrSignedToken)
	if err == nil && cookieBoxId != "" {
		decodedValue := ""
		err = p.secureCookie.Decode(SANDBOX_AUTH_COOKIE_NAME+boxIdOrSignedToken, cookieBoxId, &decodedValue)
		if err != nil {
...
func (p *Proxy) getBoxIdFromSignedPreviewUrlToken(ctx *gin.Context, boxIdOrSignedToken string, port float32, cookieDomain string) (string, error) {
	...
	encoded, err := p.secureCookie.Encode(SANDBOX_AUTH_COOKIE_NAME+boxIdOrSignedToken, boxId)
	if err != nil {
		return "", fmt.Errorf("failed to encode cookie: %w", err)
	}

	ctx.SetCookie(SANDBOX_AUTH_COOKIE_NAME+boxIdOrSignedToken, encoded, 3600, "/", cookieDomain, p.config.EnableTLS, true)
