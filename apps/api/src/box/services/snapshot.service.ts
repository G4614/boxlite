...
export class SnapshotService {
  ...
  private async handleBoxCreatedEvent(event: BoxCreatedEvent) {
    if (!event.box.snapshot) {
      return
    }

    // Update once per minute at most
    if (!(await this.redisLockProvider.lock(`snapshot:${event.box.snapshot}:update-last-used`, 60))) {
      return
    }

    const snapshot = await this.getSnapshotByName(event.box.snapshot, event.box.organizationId)
    snapshot.lastUsedAt = event.box.createdAt
    await this.snapshotRepository.save(snapshot)
  }
  ...
  async canCleanupImage(imageName: string): Promise<boolean> {
    ...
    if (box && box.state !== BoxState.DESTROYED) {
      return false
    }
