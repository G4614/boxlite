...
This directory holds `--kernel-variant net` (DinD) kernel config overlays
...
spawn time based on `BoxOptions::kernel_variant`.
...
`make libkrunfw-net` delegates the config merge and firmware build to
