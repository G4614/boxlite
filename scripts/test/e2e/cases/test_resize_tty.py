...
@pytest.mark.asyncio
async def test_resize_tty_on_tty_execution(box):
    ...
    if hasattr(result, "__await__"):
        await result
    # Tear down the exec cleanly so the fixture's box.remove doesn't race
    # against a still-running pty
    if hasattr(ex, "kill"):
        kill_res = ex.kill()
...
@pytest.mark.asyncio
async def test_resize_tty_on_non_tty_execution_raises(box):
